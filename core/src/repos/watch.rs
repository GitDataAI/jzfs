...
use chrono::Utc;
use database::{git_repo_stats, user_watch_repo};
...
impl AppCore {
    pub async fn user_watched_repo(
        &self,
        namespace: &str,
        repo_name: &str,
        session: Session,
        ...
        user_watch_repo::Entity::find()
            .filter(user_watch_repo::Column::UserId.eq(user.user_uid))
            .filter(user_watch_repo::Column::RepoId.eq(repo.uid))
            .one(&self.db)
            .await
            .ok()
            .flatten()
    ...
    pub async fn watch_repo(
        ...
        let now = Utc::now();
        let uid = Uuid::now_v7();
        let active = user_watch_repo::ActiveModel {
            uid: Set(uid),
            user_id: Set(user.user_uid),
            repo_id: Set(repo.uid),
            created_at: Set(now.into()),
        };
        user_watch_repo::Entity::insert(active).exec(&txn).await?;
        if let Some(state) = git_repo_stats::Entity::find()
            .filter(git_repo_stats::Column::RepoUid.eq(repo.uid))
            .one(&txn)
            .await?
        {
            let mut active = state.clone().into_active_model();
            active.watches = Set(state.watches + 1);
    ...
    pub async fn unwatch_repo(
        ...
        let user = self
            ...
            .await
            .map_err(|_| sea_orm::DbErr::Custom("Not login".into()))?;
        user_watch_repo::Entity::delete_many()
            .filter(user_watch_repo::Column::UserId.eq(user.user_uid))
            .filter(user_watch_repo::Column::RepoId.eq(repo.uid))
            .exec(&txn)
            .await?;
    ...
    pub async fn users_watch_repos(
        &self,
        username: &str,
        paginator: Paginator,
        ...
        let repos = user_watch_repo::Entity::find()
            .filter(user_watch_repo::Column::UserId.eq(user.uid))
        ...
        let total = user_watch_repo::Entity::find()
            .filter(user_watch_repo::Column::UserId.eq(user.uid))
            .count(&self.db)
            .await?;
        Ok(json!({
