...
use uuid::Uuid;
use crate::api::dto::user_dto::UserKeyList;
use crate::metadata::model::users::users_keys;
...
impl UserService {
    pub async fn get_users_by_pubkey(&self, public_key: &PublicKey) -> anyhow::Result<Uuid>{
        ...
        let key_model = users_keys::Entity::find()
            .filter(
                users_keys::Column::Pubkey.contains(public_key_base64)
            )
            .one(&self.db)
            .await?;
        match key_model {
    ...
    pub async fn list_key(&self, user_id: Uuid) -> anyhow::Result<Vec<UserKeyList>>{
        let key_list = users_keys::Entity::find()
            .filter(users_keys::Column::UserId.eq(user_id))
            .all(&self.db)
            .await?;
        let mut keys = vec![];
    ...
    pub async fn add_key(&self, user_id: Uuid, name: String, pubkey: String) -> anyhow::Result<()>{
        let arch = users_keys::ActiveModel{
            uid: Set(Uuid::new_v4()),
            user_id: Set(user_id),
            pubkey: Set(pubkey),
            name: Set(name),
    ...
    pub async fn remove_key(&self, user_id: Uuid, uid: Uuid) -> anyhow::Result<()>{
        let arch = users_keys::Entity::find_by_id(uid)
            .filter(users_keys::Column::UserId.eq(user_id))
            .one(&self.db)
            .await?;
        if arch.is_none(){
