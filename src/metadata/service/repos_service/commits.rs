...
impl RepoService {
    ...
    pub async fn list_commits(&self,repo_id: Uuid,branch: String, offset: u64, size: u64) -> anyhow::Result<Vec<repo_commit::Model>>{
        let model = repo_branch::Entity::find()
            .filter(repo_branch::Column::RepoId.eq(repo_id))
            .filter(repo_branch::Column::Branch.eq(branch))
            .one(&self.db)
            .await?;
        if model.is_none(){
