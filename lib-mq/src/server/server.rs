...
use crate::server::email::{EmailEvent, EmailType};
...
impl AppKafkaConsumer {
    ...
    pub async fn topic(&mut self, topics: Vec<String>) -> std::io::Result<()>{
        ...
        consumer.subscribe(&topics.iter().map(|x| x.as_str()).collect::<Vec<&str>>())
            .map_err(|x|{
    ...
    pub async fn listen(self) {
        ...
        tokio::spawn(async move {
            let email = EmailEvent::new(self.nacos.clone()).await
                .map_err(|e| {
                    error!("{}", e);
                ...
                match consumer.recv().await {
                    Ok(message) => {
                        ...
                        match Topic::try_from_string(topic.to_string()){
                            Some(topic)=>{
                                info!("Received new work from {}, ready to start execution", topic.to_string());
                                match topic {
                                    Topic::Email => {
                                        if let Ok(email_type) = serde_json::from_slice::<EmailType>(&payload) {
