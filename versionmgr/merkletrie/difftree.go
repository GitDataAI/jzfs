...
func DiffTree(
	...
	return DiffTreeContext(context.Background(), fromTree, toTree)
}
...
func DiffTreeContext(ctx context.Context, fromTree, toTree noder.Noder) (Changes, error) {
	ret := NewChanges()

	ii, err := newDoubleIter(fromTree, toTree)
	if err != nil {
		return nil, err
	}
...
func diffNodesSameName(changes *Changes, ii *doubleIter) error {
	...
	case status.bothAreDirs:
		isDirChildrenSame, err := isDiffChildrenSame(from.Last(), to.Last())
		if err != nil {
			return err
		}
		if isDirChildrenSame {
			changes.Add(Change{
				From: from,
				To:   to,
			})
...
func isDiffChildrenSame(from, to noder.Noder) (bool, error) {
	...
	for index, fromChild := range fromChildren {
		toChild := toChildren[index]
		if !fromChild.Equal(toChild) {
			return false, nil
		}
	}
	return true, nil
