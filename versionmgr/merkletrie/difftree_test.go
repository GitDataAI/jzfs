...
func (t diffTreeTest) innerRun(c *C, context string, reverse bool) {
	...
	results, err := merkletrie.DiffTree(a, b)
	c.Assert(err, IsNil, comment)
...
func (t diffTreeTest) innerRunCtx(c *C, context string, reverse bool) {
	...
	results, err := merkletrie.DiffTreeContext(ctx.Background(), a, b)
	c.Assert(err, IsNil, comment)
...
func (checker *changesEqualsChecker) Check(params []interface{}, _ []string) (result bool, error string) {
...
func (s *DiffTreeSuite) TestCancel(c *C) {
	...
	cancel()
	results, err := merkletrie.DiffTreeContext(context, a, b)
