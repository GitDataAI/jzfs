...
use crate::server::email::{EmailAddParma, MainEmailUpdate};

pub async fn main_email_can(
    session: Session,
    ...
    -> impl Responder
{
    ...
    match state.user_email_delete(users.uid, parma.into_inner().email).await {
        Ok(_) => AppWrite::ok(()),
        Err(_) => AppWrite::unauthorized("用户未登录".to_string()),
    }
}

pub async fn main_email_list(
    session: Session,
    state: web::Data<AppUserState>
)
    -> impl Responder
    ...
    -> impl Responder
{
    ...
    match state.user_email_set_primary(users.uid, parma.into_inner().email).await {
        Ok(_) => AppWrite::ok(()),
        Err(_) => AppWrite::unauthorized("用户未登录".to_string()),
    }
}
    ...
    -> impl Responder
{
    ...
    match state.user_email_set_no_primary(users.uid, parma.into_inner().email).await {
        Ok(_) => AppWrite::ok(()),
        Err(_) => AppWrite::unauthorized("用户未登录".to_string()),
    }
}