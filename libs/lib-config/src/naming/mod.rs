...
impl AppNaming {
    pub fn new(
        client : Arc<Box<dyn nacos_sdk::api::naming::NamingService>>,
        name : String,
    ) -> Self {
        AppNaming {
            client,
    ...
    pub async fn register(
        ...
        self.client
            .register_instance(key.to_string(), Some(groups.to_string()), instance)
            .await?;
    ...
    pub async fn unregister(&self) -> anyhow::Result<()> {
        if let Some(key) = &self.name_key {
            self.client
                .deregister_instance(key.to_string(), None, ServiceInstance::default())
                .await?;
        }

        Ok(())
    }
    pub async fn list_http_server(
        &self,
        group : Vec<&str>,
    ) -> anyhow::Result<Vec<HttpServiceNode>> {
        let mut result = HashMap::new();
        for idx in group {
            let (res, _) = self
                .client
                .get_service_list(0, i32::MAX, Some(idx.to_string()))
                .await?;
