...
use deadpool_redis::cluster::ClusterClient;
use deadpool_redis::cluster::ClusterConnection;
...
impl actix_session::storage::SessionStore for ClusterMaster {
    ...
    ) -> impl Future<Output = Result<Option<actix_session::storage::interface::SessionState>, LoadError>>
    {
        let mut client = self.client.lock().unwrap();
        let key = session_key.as_ref().to_string();
        async move {
            let data = client.get::<String, String>(key).await;
            match data {
                Ok(bytes) => {
    ...
    fn save(
        &self,
        session_state : actix_session::storage::interface::SessionState,
        ttl : &time::Duration,
        ...
        async move {
            ...
            client
                .set::<String, String, String>(key.clone(), bytes)
                .await
                .map_err(|e| SaveError::Other(e.into()))?;
            client
                .expire::<String, bool>(key, ttl.whole_seconds())
                .await
                .map_err(|e| SaveError::Other(e.into()))?;
            Ok(session_key)
        }
    ...
    fn update(
        ...
        async move {
            ...
            if !exists {
                return Err(UpdateError::Serialization(anyhow::anyhow!(
                    "Session not found"
                )));
            }
    ...
    fn delete(&self, session_key : &SessionKey) -> impl Future<Output = Result<(), anyhow::Error>> {
        ...
        async move {
            client.del::<String, String>(key).await?;
            Ok(())
        }
    }
}
