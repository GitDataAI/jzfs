...
impl AppConfig {
    pub async fn postgres_config(
        &self,
        kind : AppPostgresConfigKind,
    ) -> std::io::Result<AppPostgresConfig> {
        let mut idx = 0;
        loop {
            ...
            if let Ok(data) = self
                .client
                .get_config(data_id, "postgres".to_string())
                .await
            {
    ...
    pub async fn postgres_connect(
        &self,
        kind : AppPostgresConfigKind,
    ) -> std::io::Result<sea_orm::DatabaseConnection> {
        ...
        db_config
            ...
            .connect_lazy(true)
            .sqlx_logging(false);
        let client = sea_orm::Database::connect(db_config)
            .await
