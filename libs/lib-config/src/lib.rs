...
impl AppNacos {
    pub fn parse_env() -> std::io::Result<AppNacosBuilder> {
        let env = env::var("NACOS").map_err(|x| io::Error::new(io::ErrorKind::Other, x))?;

        let parts : Vec<&str> = env.split("://").collect();
        let server_name = parts[0].to_string();
        let auth_and_host_port_namespace = parts[1];

        let auth_and_host_port_namespace_parts : Vec<&str> =
            auth_and_host_port_namespace.split('@').collect();
        if auth_and_host_port_namespace_parts.len() != 2 {
            return Err(io::Error::new(
                io::ErrorKind::InvalidInput,
                "Invalid NACOS URL format (missing @)",
            ));
        }
        ...
        if auth_parts.len() != 2 {
            return Err(io::Error::new(
                io::ErrorKind::InvalidInput,
                "Invalid NACOS URL format (missing username:password)",
            ));
        }
        ...
        if host_port_namespace_parts.len() < 2 {
            return Err(io::Error::new(
                io::ErrorKind::InvalidInput,
                "Invalid NACOS URL format (missing namespace)",
            ));
        }
        ...
        if host_port_parts.len() != 2 {
            return Err(io::Error::new(
                io::ErrorKind::InvalidInput,
                "Invalid NACOS URL format (missing port)",
            ));
        }
    ...
    pub fn from_env() -> io::Result<Self> {
        ...
        let naming_service =
            nacos_sdk::api::naming::NamingServiceBuilder::new(client_props.clone())
                .enable_auth_plugin_http()
                .build()
                .map_err(|err| io::Error::new(io::ErrorKind::Other, err))?;
        let config_service =
            nacos_sdk::api::config::ConfigServiceBuilder::new(client_props.clone())
                .enable_auth_plugin_http()
                .build()
                .map_err(|err| io::Error::new(io::ErrorKind::Other, err))?;
...
mod tests {
    ...
    fn test_parse_env() {
        ...
        let builder = AppNacos::parse_env().expect("Failed to parse NACOS environment variable");
