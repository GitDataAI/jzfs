...
mod tests {
    ...
    async fn repos_sql() {
        ...
        manager
            .push(
                repos::ReposMigration::create().to_string( PostgresQueryBuilder{})
            );
        manager
            .push(
                branchs::BranchMigration::create().to_string( PostgresQueryBuilder{})
            );
        manager
            .push(
                commits::CommitMigration::create().to_string( PostgresQueryBuilder{})
            );
        manager
            .push(
                labels::LabelsMigration::create().to_string( PostgresQueryBuilder{})
            );
        manager
            .push(
                license::LicenseMigration::create().to_string( PostgresQueryBuilder{})
            );
        manager
            .push(
                blobtree::BlobTreeMigration::create().to_string( PostgresQueryBuilder{})
            );
