...
use lib_config::config::redis::ClusterMaster;
...
async fn main() -> std::io::Result<()> {
    ...
    naming
        .register(PORT.clone() as i32, "gateway", 1)
        .await
        ...
        loop {
            if let Ok(mut list) = naming.clone().list_http_server(vec!["api", "web"]).await {
                ...
                if http_server_list != list {
                    info!(
                        "Receive service configuration updates and start preparing the routing registration process"
                    );
                    ...
                    for idx in list.iter_mut() {
                        let ips = idx
                            ...
                            .collect::<Vec<String>>();
                        let fast_ip = ping::find_fastest_dns_server(
                            &ips.iter().map(|x| x.as_str()).collect::<Vec<&str>>(),
                            idx.port as u16,
                        )
                        .await;
    ...
    tokio::spawn(async move {
        async fn run(
            redis : ClusterMaster,
            server_list : Vec<HttpServiceNode>,
        ) -> (ServerHandle, Server) {
            let serve = HttpServer::new(move || {
                App::new()
                    ...
                    .wrap(
                        ...
                        .session_lifecycle(
                            PersistentSession::default().session_ttl(Duration::days(30)),
                        )
        ...
        tokio::spawn(async move {
            ...
            while let Some(list) = http_server_rx.recv().await {
                info!(
                    "Detected that the configuration center API service update gateway is about to restart"
                );
                server_list.clear();
