...
func NewAdapter(_ context.Context, params params.Azure) (*Adapter, error) {
	log.With("type", "azure").Info("initialized blockstore adapter")
...
func (a *Adapter) Download(ctx context.Context, obj block.ObjectPointer, offset, count int64) (io.ReadCloser, error) {
	...
	if err != nil {
		log.Errorf("failed to get azure blob from container %s key %s %v", container, blobURL, err)
		return nil, err
	}
...
func (a *Adapter) Copy(ctx context.Context, sourceObj, destinationObj block.ObjectPointer) error {
	...
	for {
		select {
		case <-ctx.Done():
			log.With("copy_progress", progress).Warn("context canceled, aborting copy")
			...
			if err != nil {
				log.Errorf("failed to abort copy %v", err)
			}
			return ctx.Err()
		...
		case <-time.After(asyncPollInterval):
			...
			case blob.CopyStatusTypeSuccess:
				log.With("object_properties", p).Debug("Async copy successful")
				return nil
			...
			case blob.CopyStatusTypePending:
				log.With("copy_progress", progress).Debug("Copy pending")
